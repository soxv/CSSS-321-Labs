...
- Key Questions in This Week
  ...
  - How can we use potential outcome to identify the causal effect of a specific policy (treatment)?
  - Why can randomized experiment help us identify causal effect?
  - How do we get the causal effect in a randomized experiment?
...
- Causal Inference as a missing data problem
  ...
  - Two scenarios
    - Experiment: researcher can control and know the assignment of treatment
    - Observational studies: researcher cannot control and cannot exactly know the assignment of treatment
...
:::{.callout-note title="Statistical Jargons: Estimand, Estimator, and Estimate"}
...
## Why Randomize Treatment Assignment?
...
### Unbiasedness of $\hat{\tau}^\text{dif}$
...
Hence, the expectation of $\hat{\tau}^\text{dif}$ is[^3]
...
### Uncertainty of $\hat{\tau}^\text{dif}$

We can prove that the sampling variance of $\hat{\tau}^\text{dif}$ is
$$
\mathbb{V}_{T}\left(\bar{Y}_{\mathrm{t}}^{\mathrm{obs}}-\bar{Y}_{\mathrm{c}}^{\mathrm{obs}}\right)=\frac{S_{c}^{2}}{n_0}+\frac{S_{t}^{2}}{n_1}-\frac{S_{t c}^{2}}{n}
$$
where $S_c^2$ and $S_t^2$ are the variance of potential outcomes $Y_i(0)$ and $Y_i(1)$ in the sample, defined as:
$$
S_{c}^{2}=\frac{1}{n-1} \sum_{i=1}^{n}\left(Y_{i}(0)-\bar{Y}(0)\right)^{2}, \quad \text { and } \quad S_{t}^{2}=\frac{1}{n-1} \sum_{i=1}^{n}\left(Y_{i}(1)-\bar{Y}(1)\right)^{2}
$$
...
&=\frac{1}{n-1} \sum_{i=1}^{N}\left(Y_{i}(1)-Y_{i}(0)-\tau_\text{SATE}\right)^{2}
\end{align}
$$
which is unobservable. This term will diminish to zero when treatment effect is constant across unit. Because the term $S_{tc}^2$ is a non-negative value, we can drop this term and use the following estimator as a conservative upper bound for the sampling variance of $\hat{\tau}^\text{dif}$:

$$
\mathbb{V}_{T}\left(\bar{Y}_{\mathrm{t}}^{\mathrm{obs}}-\bar{Y}_{\mathrm{c}}^{\mathrm{obs}}\right) \leq \frac{S_{c}^{2}}{n_0}+\frac{S_{t}^{2}}{n_1}
$$

For detailed proof, see Imbens and Rubin (2015) Chapter 6.

## (Fisher's) Permutation Inference
...
## Neyman's Approach vs. Fisher's Approach
...
## 1. Check Balance After Randomization
...
aggregate(
  ...
  data = transphobia,
  FUN = mean
) # check balance for multiple continuous variables
...
# take the baseline outcome as an example
tapply(transphobia$therm_trans_t0, transphobia$treat_ind, mean) |> # use tapply() to calculate the means of each group
  rev() |> # reverse the order from 0 1 to 1 0
  diff() # calculate the difference in means

# a more compact way to calculate difference in means for baseline outcome and later periods
library(tidyverse) # load the package tidyverse so that we can use the pipe operator `%>%`
aggregate(
  cbind(therm_trans_t0, therm_trans_t1, therm_trans_t2) ~ treat_ind,
  data = transphobia,
  FUN = mean
) %>% # create a matrix that shows group means
  rbind(., apply(., MARGIN = 2, diff)) # use apply() to calculate differences in each column;
                                       # and use rbind() to concatenate the row of differences to previous matrix that shows group means;
                                       # dot "." means the object generated from previous code.
...
mean(treated_female$therm_trans_t0) - mean(control_female$therm_trans_t0)

treated_male <- subset(transphobia, treat_ind == 1 & female == 0)
control_male <- subset(transphobia, treat_ind == 0 & female == 0)
mean(treated_male$therm_trans_t0) - mean(control_male$therm_trans_t0)

# method 2: a more compact way to calculate CATE for baseline outcome and later periods
aggregate(
  cbind(therm_trans_t0, therm_trans_t1, therm_trans_t2) ~ treat_ind + female,
  data = transphobia,
  FUN = mean
) %>%
  rbind( # use rbind() to concatenate difference in means to the previous matrix showing group means
    ., apply(., MARGIN = 2, function(x) {
      c(
        diff(x[1:2]), # calculate difference in means between the first two rows (i.e. treated and control within male group)
        diff(x[3:4]) # calculate difference in means between the last two rows (i.e. treated and control within female group)
      )
    })
  )
...
obs_dif <- aggregate(
  ...
  apply(., MARGIN = 2, diff) %>%
  .[-1] # we want to drop the first element, which is useless in our analysis
...
for (i in 1:1000) {
  ...
  rand_dif[i, ] <- aggregate( # calculate difference in means between randomly split groups
