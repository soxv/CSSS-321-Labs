...
- Key Questions in This Week
  ...
  - [How do we get the causal effect in a randomized experiment?](#slide-neyman)
...
## Key Assumptions (skip; good to know)
...
## (Fisher's) Permutation Inference (skip)
...
## Neyman's Approach vs. Fisher's Approach (skip)
...
Let's open the `.Rmd` file from Canvas! (review how to use `setwd()` or open `R` project)
...
## Fisher's Exact $p$-value Approach (skip)
